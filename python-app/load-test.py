...
def get_books(max_num_rows=100, max_length_content_display=16, DEBUG='OFF'):
  ...
  # get books 
  cursor.execute("SELECT id, LEFT(content,%s) FROM books LIMIT %s", 
                 (max_length_content_display, max_num_rows,))
  # print books
  if DEBUG == 'ON':
...
def insert_books_long_text(id=10001,item_size=10,DEBUG="OFF"):
  ...
  # execute query
  cursor = connection.cursor()
  cursor.execute("INSERT INTO books(id, content) VALUES(%s, %s)", (id, content,))
  # must commit 
  connection.commit()
  # close connection
  connection.close()
...
def load_test_read_only_books(num_loop, num_thread, max_num_row,max_length_content_display=16, DEBUG="OFF"):
  """
  ...
  for k in range(num_loop):
    ...
    with ThreadPoolExecutor(max_workers=num_thread) as executor:
      for id in range(num_thread):
        executor.submit(get_books, max_num_row, max_length_content_display, DEBUG)
